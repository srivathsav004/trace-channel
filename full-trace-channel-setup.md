...
## Phase 2 — Create a custom network folder

Do not modify `test-network` directly. Clone it to your own folder.

```bash
cd ~/fabric-dev/fabric-samples
cp -r test-network trace-network
cd trace-network
```

For this documented run, the working folder is `trace-network2` (same idea, different folder name):
...
## Phase 3 — Add third organization (Manufacturer / Distributor / Retailer)

Official guides may refer to:

- `organizations/cryptogen/crypto-config.yaml`

But this repo layout uses split peer-org files, so apply equivalent changes here:
...
## Phase 4 — Configure 3 RAFT orderers

Edit:

- `organizations/cryptogen/crypto-config-orderer.yaml`

Use these orderer hosts:
...
## Phase 5 — Update `configtx.yaml` (orgs + RAFT consenters)

Edit:

- `configtx/configtx.yaml`

Define organizations and anchor peers:

- `ManufacturerMSP` -> `peer0.manufacturer.example.com:7051`
- `DistributorMSP` -> `peer0.distributor.example.com:9051`
- `RetailerMSP` -> `peer0.retailer.example.com:11051`

Set RAFT consenters to:
